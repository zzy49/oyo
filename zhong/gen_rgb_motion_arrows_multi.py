#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""rgb_warped 运动方向箭头 —— 多背景帧 × 一种方法一张图 (供手动合成).

在 v2 序列里选 4 张帧号拉开距离的背景图, 每张背景帧:
  - 相同背景 + 相同锚点 + 相同编号 (Top 20 运动点)
  - 每种方法 (GT / Monodepth2 / ManyDepth / Lite-Mono / Ours) 单独一张图
  - 只画该方法的运动方向箭头 (noscale, 各用自身 global_scale 物理尺度, 不做 median scaling)
  - 同一背景帧下 5 张图除箭头外完全一致, 可直接叠加/手动合成
...
OUT_DIR = os.path.join(BASE, 'ours_rerun', 'analysis', 'rgb_motion_arrows_multi')
...
BG_FRAMES = [15, 35, 75, 105]  # 4 张背景帧, 间隔拉开, 35 替代 45(高光过多)
...
def process_bg(bg_frame, trajs, gt_dep, w2c, method_dep):
    out_sub = os.path.join(OUT_DIR, f'bg{bg_frame:03d}')
    ...
    for name in ALL:
        ...
        ax.axis('off')
        plt.tight_layout()
        out_png = os.path.join(out_sub, f'arrows_{name}.png')
        fig.savefig(out_png, dpi=110, bbox_inches='tight')
        plt.close(fig)
        print(f'  Saved: {out_png}')

    # 方向角差统计 (各方法 vs GT)
    print(f'  方向角差 (度, vs GT):')
    for m in METHODS:
        diffs = []
        for r in top:
            vg, vm = r[1]['GT'], r[1][m]
            ...
            diffs.append(d)
        if diffs:
            print(f'    {m:<12} median={np.median(diffs):.1f}, mean={np.mean(diffs):.1f}')
...
def main():
    ...
    for bg_frame in BG_FRAMES:
        process_bg(bg_frame, trajs, gt_dep, w2c, method_dep)

    print(f'\n全部输出目录: {OUT_DIR}')
    print(f'共 {len(BG_FRAMES) * len(ALL)} 张图')
