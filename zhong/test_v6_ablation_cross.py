...
# -*- coding: utf-8 -*-
"""
V6 方法消融实验 - F1 x M1 交叉验证补充
只跑缺失变体: w/o F1+M1 (关 F1 且关 M1, F2 保持开启)

F1 x M1 交叉表 (F2=开启):
                  M1=开              M1=关
  F1=开      Ours (M1+F1+F2)      w/o M1
  F1=关      w/o F1               w/o F1+M1   <-- 本次新增

其余 3 个变体结果复用 ablation_results.json
"""
...
# 本次只跑缺失变体
VARIANTS = [
    {'name': 'w/o F1+M1', 'use_motion_encoder': False, 'use_displacement_filter': False, 'use_scale_corr': True},
]

# 已有结果 (来自 ablation_results.json)
PREV_RESULTS = {
    'Ours (M1+F1+F2)': {'use_motion_encoder': True,  'use_displacement_filter': True},
    'w/o M1':          {'use_motion_encoder': False, 'use_displacement_filter': True},
    'w/o F1':          {'use_motion_encoder': True,  'use_displacement_filter': False},
}
...
def main():
    ...
    if os.path.exists(prev_path):
        with open(prev_path, 'r') as fp:
            prev = json.load(fp).get('variants', {})
        print('[2/2] 已加载既有消融结果 (3 变体)\n')
    else:
        print('[2/2] 未找到既有结果, 仅跑新变体\n')
    ...
    # 汇总: F1 x M1 交叉表 (F2=开启)
    print('\n' + '=' * 100)
    print('  F1 x M1 交叉验证结果 (F2 开启, 标准 EPnP)')
    print('=' * 100)
    header = (f"  {'变体':<18} {'ATE':>8} {'RPE-T':>8} {'RPE-R':>8} "
              f"{'ScaleErr':>10} {'umeyama':>10} {'Success':>9}")
    print(header)
    print('  ' + '-' * 92)
    order = ['Ours (M1+F1+F2)', 'w/o M1', 'w/o F1', 'w/o F1+M1']
    for name in order:
        m = results.get(name)
        if m is None:
            print(f"  {name:<18} {'缺失':>8}")
        elif 'error' in m:
    ...
    # 交叉表矩阵
    print('\n  F1 x M1 交叉表 (ATE / ScaleErr):')
    print('  ' + '-' * 60)
    print(f"  {'':<10} {'M1=开':>22} {'M1=关':>22}")
    print(f"  {'F1=开':<10} {_fmt(results.get('Ours (M1+F1+F2)')):>22} {_fmt(results.get('w/o M1')):>22}")
    print(f"  {'F1=关':<10} {_fmt(results.get('w/o F1')):>22} {_fmt(results.get('w/o F1+M1')):>22}")

    # 保存
    out_path = os.path.join(OUT_DIR, 'ablation_cross_results.json')
...
def _fmt(m):
    ...
    if 'error' in m:
        return 'ERROR'
    return f"{m['ate_mean']:.2f} / {m['scale_error_pct']:.2f}%"
