...
# -*- coding: utf-8 -*-
"""
多模型 VO 对比测试 —— 基于 test_v6_dyendovo.py 管线。

直接导入 test_v6_dyendovo 的 run_vo_sequence / evaluate_trajectory,
替换不同深度模型进行公平对比。

用法:
  python test_v6_multi_model_compare.py

管线: 与 test_v6_dyendovo.py baseline + pnp_scale 零配置完全一致
  - 5 模型: Monodepth2 / ManyDepth / Lite-Mono / Ours / Ours+Enhanced
  - LoFTR 缓存匹配 (cache) —— 所有模型共用同一组匹配
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'vo_depth_compare')
...
MODEL_CONFIGS = [
    ...
    {
        'name': 'Ours+Enhanced',
        'path': r'C:\Users\Administrator\tmp\mdp\models\weights_19',
        'type': 'ours_enhanced',
    },
]
...
def compute_rpe(abs_poses, gt_poses_std):
    ...
    abs_poses:  list of (4,4)  T_world_cam 位姿
    ...
    for i in range(n - 1):
        ...
        rot_err = float(np.arccos(np.clip((np.trace(dR) - 1) / 2, -1.0, 1.0)) * 180 / np.pi)
...
def main():
    ...
    print(f"设备: {device}")
    print(f"序列: {SEQ_NAME}")
    print(f"管线: test_v6_dyendovo baseline + pnp_scale (零配置)")
    ...
    # ── 快速汇总表 ──
    print(f"\n{'=' * 90}")
    print(f"  5 模型对比汇总 (test_v6_dyendovo pipeline, baseline + pnp_scale)")
    print(f"{'=' * 90}")
