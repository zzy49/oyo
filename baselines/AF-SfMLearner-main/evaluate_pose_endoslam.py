"""AF-SfMLearner 位姿网络在 EndoSLAM 上的位姿评估（Umeyama 7 自由度 ATE 口径）。

流程：
1. 读 EndoSLAM 图像（Frames_jpg/image_XXXX.jpg），按 stride 采样
2. 读 GT 绝对位姿 CSV（tX,tY,tZ,rX,rY,rZ,rW，单位米）
3. ResnetEncoder(18,2) + PoseDecoder 预测相邻帧相对位姿
4. 累积为绝对轨迹
...
H, W = 256, 320  # AF-SfMLearner SCARED 评估默认输入尺寸
...
def main():
    ...
    ap.add_argument("--scene", default="UnityCam/Colon",
                    help="UnityCam/Colon | UnityCam/Small Intestine | UnityCam/Stomach")
    ...
    with torch.no_grad():
        for i in range(n - 1):
            img0 = load_img(imgs[i]).to(device)
            img1 = load_img(imgs[i + 1]).to(device)
            all_color_aug = torch.cat([img1, img0], 1)  # 与官方 evaluate_pose.py 一致
            features = [pose_encoder(all_color_aug)]
            axisangle, translation = pose_decoder(features)
            T = transformation_from_parameters(axisangle[:, 0], translation[:, 0]).cpu().numpy()
            pred_poses.append(T[0])
    ...
    np.savez_compressed(os.path.join(out_dir, f"pred_pose_afsl_{scene_short}.npz"),
                        data=np.array(pred_poses))
    np.savez_compressed(os.path.join(out_dir, f"gt_poses_{scene_short}.npz"), data=gt)
    print(f"已保存: pred_pose_afsl_{scene_short}.npz ({len(pred_poses)},4,4) + gt_poses_{scene_short}.npz ({gt.shape[0]},4,4)")
    ...
    print(f"\n===== AF-SfMLearner on EndoSLAM {args.scene} =====")
